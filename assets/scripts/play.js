document.addEventListener("turbo:load", () => {
    ...
    if (winner === null) {
        if (turnPlayerType === "enemy") {
            if (enemyCardPlayed) {
                setTimeout(() => {
                    enemyPlaceCard(turnId, enemyCardPlayed);
                    setTimeout(() => {
                        window.location.href = enemyBaseUrl;
                    }, 200);
                }, 3000);
            } else if (cardPicked === false) {
                setTimeout(() => {
                    pickCard(turnId, deck);
                    setTimeout(() => (window.location.href = deckBaseUrl), 200);
                }, 3000);
            } else {
...
function pickCard(turnId, deck) {
    ...
    console.log(allPlayersElement[1]);
    let finalTransform;
...
function enemyPlaceCard(turnId, enemyCardPlayedId) {
    const animationCardContainer = document.querySelector(
        ".game-animation-container .card-container",
    );
    const enemyElement = document.querySelector(`#enemy-cards${turnId}`);
    const card = enemyElement.querySelector(`#card-${enemyCardPlayedId}`);
...
function placeCardOnDiscard(cardContainer) {
